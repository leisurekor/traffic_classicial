...
![merge01 summary chart](assets/merge01_metric_summary.png)
...
- 阈值分位数（threshold percentile）：`95.0`
- 学习率（learning rate）：`0.001`
- 批大小（batch size）：`2`
- 统一配置中的训练轮数（configured epochs）：`2`
- 图分数约简策略（graph score reduction）：`hybrid_max_rank_flow_node_max`
- 数据划分：`train=0.6` / `val=0.2` / `test=0.2`
- 输入样本数（rows after cleaning）：`N/A`
- 特征维数（feature dimension）：`N/A`
- 训练集 benign-only 约束：`N/A`
...
![ctu13 benchmark summary](assets/ctu13_graph_benchmark_summary.png)
...
![ctu13 pipeline chart](assets/ctu13_pipeline_distribution.png)
